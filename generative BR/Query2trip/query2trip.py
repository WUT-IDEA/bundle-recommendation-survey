...
def encoder(X,context):
    ...
    with tf.variable_scope("encoder",reuse=False):
        ...
        trans_tensor=tf.concat([tensor,space],2)
        inputenc=tf.transpose(trans_tensor,[1,0,2])        #[length,batch_size,embedding_size].
        print 'input tensor shape',inputenc
        inputenc *= inputenc**0.5
        inputenc = tf.nn.l2_normalize(inputenc)
        src_masks = tf.equal(inputenc, 0)


        inputenc += positional_encoding(inputenc, maxlen=3)
        latent_code = tf.layers.dropout(inputenc, training=True)
        for i in range(1):
            with tf.variable_scope("num_blocks_{}".format(i), reuse=tf.AUTO_REUSE):
                # self-attention
                enc = multihead_attention(queries=latent_code,
                                          keys=latent_code,
                                          values=latent_code,
                                          key_masks=None,
                                          num_heads=8,
                                          ...
                                          causality=False)
                # feed forward
                enc = ff(enc, num_units=320)
        print 'latentcode',enc
        latent_code = enc
        ...
        return latent_code, X

# =============================== Decoder ====================================== #
def decoder(tensor,X,reuse=False):
    """
    ...
    with tf.variable_scope('decoder',reuse=reuse) as scope:
        ...
        dec = tf.nn.embedding_lookup(embeddings,X)
                # src_masks = tf.math.equal(inputenc, 0)

        tensor += positional_encoding(latent_code,30)
        latent_code = tf.layers.dropout(latent_code, 0.5, training=True)
        latent_code = tf.nn.l2_normalize(latent_code)
        for i in range(1):
            with tf.variable_scope("num_blocks_{}".format(i), reuse=tf.AUTO_REUSE):
                # self-attention
                dec = multihead_attention(queries=latent_code,
                                          keys=latent_code,
                                          values=latent_code,
                                          key_masks=None,
                                          num_heads=8,
                ...
                dec = ff(dec, num_units=320)
        print 'latentcode',
        latent_code = dec
        latent_code = latent_code[-1]
        latent_code = tf.nn.l2_normalize(latent_code)
        weights = tf.transpose(latent_code)
        logits = tf.einsum('ntd,dk->ntk', latent_code, weights)  # (N, T2, vocab_size)
        predicting_logits = tf.to_int32(tf.argmax(logits, axis=-1))

        training_logits = predicting_logits

        masks = tf.sequence_mask(target_sequence_length, max_target_sequence_length, dtype=tf.float32, name='masks')
        target = X

        return latent_code, predicting_logits, training_logits, masks, target
...
def generator(z,context,reuse=False):
    ...
    with tf.variable_scope("generator"):
        ...
        latent_code = tf.layers.dropout(trans_tensor, training=True)
        for i in range(1):
            with tf.variable_scope("num_blocks_{}".format(i), reuse=tf.AUTO_REUSE):
                # self-attention
                enc = multihead_attention(queries=latent_code,
        ...
        latent_code = enc
        latent_code = latent_code[-1]
        g_code = tf.nn.l2_normalize(latent_code)
        return g_code
...
# =============================== Function ====================================== #
def autoencoder(X,de_X,context,reuse=False):
    """
    ...
    with tf.variable_scope("autoencoder"):
        latent = encoder(X, context)
        output_, predicting_logits_, training_logits_, masks_, target_ = decoder(latent, de_X,reuse=tf.AUTO_REUSE)
    return training_logits_, masks_, target_, latent, predicting_logits_
...
def pad_time_batch(time_batch):
    ...
    - pad_int: <PAD>对应索引号
    '''
    max_sentence = max([len(sentence) for sentence in time_batch])  
#    max_sentence = len(voc_poi)
...
def pad_dist_batch(dist_batch):
    ...
    - pad_int: <PAD>对应索引号
    '''
    max_sentence = max([len(sentence) for sentence in dist_batch])  
#    max_sentence = len(voc_poi)
...
training_logits_,masks_,target_,real_code,predicting_logits_,latent,output_=autoencoder(input_X,input_X_de,context)
#real_code =encoder(input_X,context)
g_code=generator(z,z_context) #flat c
#print g_code
...
gmean = np.mat(g_code)
rmean = np.mat(real_code)
wdis = np.mean(gmean) - np.mean(rmean)

real=np.eye(batch_size)
sim = tf.matmul(real_code,tf.transpose(g_code))

#weight=np.random.randint(0,1,20)
sim = tf.nn.softmax(sim)
CL_loss =tf.keras.losses.categorical_crossentropy(real,sim)

print "sim==",K.print_tensor(sim)
...
z_prior_mean = Dense(latent_dim)(g_code)
z_log_var = Dense(latent_dim)(real_code)
z_mean = Dense(latent_dim)(real_code)
epsilon = K.random_normal(shape=(K.shape(z_mean)[0], latent_dim))
z_mean = z_mean + K.exp(z_log_var / 2) * epsilon
...
#AE_loss=tf.losses.mean_squared_error(training_logits_,target_, masks_)##############
AE_loss=tf.losses.mean_squared_error(latent,output_)
...
#set optimizer for each moduleautoencoder
...
CL_op=tf.train.AdadeltaOptimizer(learning_rate=CL_lr)
...
CL_grad = CL_op.compute_gradients(CL_loss,encoder_variables)
...
update_CL=CL_op.apply_gradients(CL_grad)

#reconstruction encoder_state g_state,
with tf.variable_scope("autoencoder"):
    toutput_, tpredicting_logits_, ttraining_logits_, tmasks_, ttarget_ = decoder(real_code, input_X_de, reuse=True)
    foutput_, fpredicting_logits_, ftraining_logits_, fmasks_, ftarget_ = decoder(g_code, input_X_de, reuse=True)
def train(data):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(train_iters):
            ...
            while step < len(encoder_train) // batch_size:
                ...
                _, cl_loss = sess.run([update_CL, CL_loss], feed_dict={target_sequence_length: pad_source_lengths,
                                                                       input_X: encode_batch, input_X_de: decode_batch,
                                                                       z: z_in, z_t: z_time, z_d1: z_dist1,
                                                                       z_d2: z_dist2, input_t: input_time_,
                                                                       input_d1: dist_1_, input_d2: dist_2_, z: z_in,
                                                                       z_t: z_time, z_d1: z_dist1, z_d2: z_dist2,
                                                                       keep_prob: 0.5})
                #                _, kl_loss = sess.run([update_KL, KL_loss], feed_dict={target_sequence_length: pad_source_lengths,
                #                                                                       input_X: encode_batch, input_X_de: decode_batch,
                #                                                                       z: z_in, z_t: z_time, z_d1: z_dist1,
                #                                                                       z_d2: z_dist2, input_t: input_time_,
                #                                                                       input_d1: dist_1_, input_d2: dist_2_, z: z_in,
